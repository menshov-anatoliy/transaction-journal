...
- [ ] 6.1 Читать `Sync:MaxBackfillDepthDays` (дефолт 730) и `Sync:ExtraOptionBaseCoins` (дефолт пуст) в `Program.cs` при сборке опций движков; проверить: приложение стартует без секции `Sync` в конфигурации, значения передаются в опции
- [ ] 6.2 Запустить `dotnet test` по всем тестам проекта и убедиться в зелёном прогоне; проверить grep-аудитом, что новые traceability-метки сопровождаются человекочитаемыми комментариями
